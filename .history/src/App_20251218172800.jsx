...
/*
useState hook: preserve les valeurs entre les rendus, et les changements de state déclenchent un nouveau rendu du composant.

useEffect hook: gérer les effets de bord, comme les appels API, les abonnements, ou la manipulation directe du DOM. Il s'exécute après chaque rendu du composant, en fonction des dépendances spécifiées.
utilise pour side effects au moment ou le component est complet et rendu dans le DOM.
et il a aussi une possibilite de precision de dependance pour controler quand il doit s'executer.
si la dependance est un tableau vide, il s'executera une seule fois apres le premier rendu (comportement similaire a componentDidMount).
et si on met une variable dans le tableau de dependance, il s'executera a chaque fois que cette variable change.
clear: nettoyer les abonnements ou timers pour éviter les fuites de mémoire.

useMemo hook: mémoriser des valeurs calculées entre les rendus pour éviter des recalculs coûteux a chaque rendu.
 Il ne déclenche pas de nouveau rendu lorsqu'il est mis à jour.

 useContext hook:acces aux valeurs et dcts pour tout les component pour eviter prop drilling.

 useReducer hook:pour separer la logique de gestion d'état complexe du composant lui-même.
le composant soccupe de l'affichage et de l'interaction utilisateur, tandis que le reducer gère les changements d'état en fonction des actions dispatchées.
 */
